  ...
  const renderStep = () => {
    switch (step) {
      case 'welcome':
        return (
          <div className="text-center space-y-8">
            <h1 className="text-4xl font-bold">Welcome to Mono Launcher</h1>
            <p className="text-xl text-muted-foreground">
              A minimalist, distraction-free experience to help you use your phone with intention.
            </p>
      ...
      case 'favorites':
        return (
          <div className="w-full space-y-6">
            <h2 className="text-2xl font-bold text-center">Choose Your Favorites</h2>
            <p className="text-center text-muted-foreground">Select the apps you use most often for quick access from the home screen.</p>
            <ScrollArea className="h-72 rounded-md border">
              <div className="p-4">
                {sortedApps.map(app => (
                  <div key={app.id} className="flex items-center space-x-3 py-2">
                    <Checkbox
                      id={`fav-${app.id}`}
                      checked={tempFavorites.includes(app.id)}
                      onCheckedChange={checked => handleFavoriteChange(app.id, !!checked)}
                    />
                    <Label htmlFor={`fav-${app.id}`} className="text-base font-medium">
                      {app.name}
                    </Label>
                  </div>
                ))}
      ...
      case 'focus':
        return (
            <div className="w-full space-y-6">
                <h2 className="text-2xl font-bold text-center">Essential Apps for Focus</h2>
                <p className="text-center text-muted-foreground">Which apps are absolutely essential when you need to focus?</p>
                <ScrollArea className="h-72 rounded-md border">
                    <div className="p-4">
                        {sortedApps.map(app => (
                        <div key={app.id} className="flex items-center space-x-3 py-2">
                            ...
                            />
                            <Label htmlFor={`focus-${app.id}`} className="text-base font-medium">
                            {app.name}
                            </Label>
                        </div>
                        ))}
      ...
      case 'time':
        return (
            <div className="w-full space-y-8">
                <h2 className="text-2xl font-bold text-center">Time Format</h2>
                ...
                >
                    <div className="flex items-center justify-between rounded-lg border p-4">
                        <Label htmlFor="r1" className="text-lg">12-hour (e.g., 3:00 PM)</Label>
                        <RadioGroupItem value="12" id="r1" />
                    </div>
                     <div className="flex items-center justify-between rounded-lg border p-4">
                        <Label htmlFor="r2" className="text-lg">24-hour (e.g., 15:00)</Label>
